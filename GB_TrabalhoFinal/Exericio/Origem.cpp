...
void handle_object_render(Shader& shader, Mesh object, glm::mat4& model, glm::mat4& projection, float& zoom, GLuint texture_id, Material material, float window_width, float window_height) {
	// Atualização das matrizes de modelo e projeção.
	update_object_matrix_to_move(object.getId(), model, projection, zoom, window_width, window_height);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		update_object_matrix_to_move(4, obj4_model, obj4_projection, obj4_zoom, (float)window_width, (float)window_height);
